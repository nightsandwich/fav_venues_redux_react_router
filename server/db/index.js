...
const syncAndSeed = async()=> {
    ...
    const gym = await Type.create({name: 'Gym'});
    ...
    const vital = await Venue.create({name: 'Vital', imageUrl: 'https://images.squarespace-cdn.com/content/v1/5a01fd2db1ffb6985b2a9ac5/1619815760848-MOJK5RP23L0HQKBWLFIS/VITAL_Brooklyn_MadeleineChanStanley_4_2021-6.jpg', website: 'https://www.vitalclimbinggym.com/brooklyn', typeId: gym.id, neighborhoodId: wburg.id});
    ...
    const metrorock = await Venue.create({name: 'Metrorock', visited: true, imageUrl: 'https://cdn.shortpixel.ai/spai/w_1082+q_lossy+ret_img+to_webp/https://bushwickdaily.com/wp-content/uploads/2021/04/N_moyr-yYhYbnzg5_PLs4Q-1024x768.jpg', website: 'https://www.metrorock.com/bushwick', typeId: gym.id, neighborhoodId: bushwick.id});
    ...
    const kashkar = await Venue.create({name: 'Cafe Kashkar', imageUrl: 'https://s3-media0.fl.yelpcdn.com/bphoto/uRumZek8kEPdXRIl87x1Pw/o.jpg', website: 'https://www.kashkar-cafe.com/', typeId: restaurant.id, neighborhoodId: brighton.id}); 
    const sofreh = await Venue.create({name: 'Sofreh', imageUrl: 'https://s3-media0.fl.yelpcdn.com/bphoto/mNzvusg3a8oBfht1qiMzzg/o.jpg', website: 'https://www.sofrehnyc.com/menu', typeId: restaurant.id, neighborhoodId: crownheights.id});  
    const mrkiwi = await Venue.create({name: 'Mr. Kiwi', visited: true, imageUrl: 'https://preview.redd.it/cbrwlxekgd561.jpg?auto=webp&s=80331f71fdc411b084c72366da3dca4c71410554', website: 'https://www.google.com', typeId: store.id, neighborhoodId: wburg.id});
    ...
    const goldenyearsNote1 = await Note.create({comment: 'good vibe', venueId: goldenyears.id});
    const banterNote2 = await Note.create({comment: 'good bratwurst', venueId: banter.id});
    const sofrehNote1 = await Note.create({comment: 'need to go', venueId: sofreh.id}); 
  };
